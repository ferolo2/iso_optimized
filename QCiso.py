...
listL=np.array(range(760,780,10))*0.1
#listL=[15,15.1]
a = 6.0 #20.56
#IPV=0
IPV=-1.
...
dEaux = 2.9679708818322363 - np.sqrt(1+1*(2*math.pi/68.)**2) - np.sqrt(MB**2 + 1*(2*math.pi/68.)**2)
dEaux = 0.0012
pool = Pool(processes=3)
#Etest = 2.9983
dEtest= 0.00013
for L in listL:
  Etest = np.sqrt(1+5*(2*math.pi/L)**2) + np.sqrt(MB**2 + 5*(2*math.pi/L)**2) + dEaux
  print(Etest)
  Ener=  findE(Etest,dEtest,L)
  sols.append(Ener)
  Etest =Ener
  dEaux = Ener - np.sqrt(1+5*(2*math.pi/L)**2) - np.sqrt(MB**2 + 5*(2*math.pi/L)**2)
pool.close
#print(listL)
#print(sols)
